...
function optionsframework_load_scripts( $hook ) {
	...
	if ( !wp_script_is( 'wp-color-picker', 'registered' ) ) {
		...
		$colorpicker_l10n = array(
			'clear' => __( 'Clear','HO2-eye' ),
			'defaultString' => __( 'Default', 'HO2-eye' ),
			'pick' => __( 'Select Color', 'HO2-eye' )
		);
    ...
    <div id="optionsframework-metabox" class="metabox-holder">
	    <div id="optionsframework" class="postbox">
			...
			<div id="optionsframework-submit">
				<input type="submit" class="button-primary" name="update" value="<?php esc_attr_e( '保存设置', 'HO2-eye' ); ?>" />
				<input type="submit" class="reset-button button-secondary" name="reset" value="<?php esc_attr_e( '恢复默认', 'HO2-eye' ); ?>" onclick="return confirm( '<?php print esc_js( __( '如果单击“确定”会导致之前所有的设置都丢失，确定要这样做吗？', 'HO2-eye' ) ); ?>' );" />
				<div class="clear"></div>
			</div>
			</form>
		</div> <!-- / #container -->
...
function optionsframework_validate( $input ) {
	...
	if ( isset( $_POST['reset'] ) ) {
		add_settings_error( 'options-framework', 'restore_defaults', __( '已恢复默认设置！', 'HO2-eye' ), 'updated fade' );
...
function optionsframework_save_options_notice() {
	add_settings_error( 'options-framework', 'save_options', __( '设置已保存！', 'HO2-eye' ), 'updated fade' );
}
